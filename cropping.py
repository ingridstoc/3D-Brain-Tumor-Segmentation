...
def _find_case_paths(case_dir: Path):
    t1_candidates = sorted(case_dir.glob("*t1*.nii*"))
    seg_candidates = sorted(case_dir.glob("*seg*.nii*"))

    if not t1_candidates:
        raise FileNotFoundError(f"No T1 file found in {case_dir}")
    if not seg_candidates:
        raise FileNotFoundError(f"No SEG file found in {case_dir}")

    return t1_candidates[0], seg_candidates[0]
...
def process_case_from_disk(
    ...
    case_id = case_dir.name
    t1_path, seg_path = _find_case_paths(case_dir)
...
def process_case_worker(args):
    case_dir, out_dir, out_shape, debug_dir, do_debug, base_seed = args
    case_id = case_dir.name
    try:
